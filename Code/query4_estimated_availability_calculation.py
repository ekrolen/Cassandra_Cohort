...
availability_data = pd.read_csv('calendar.csv')
...
for listing in listing_identifiers:
    ...
    listing_availability = availability_data[availability_data
                                             ['listing_id'] == listing]

    # iterate through each day for the listing
    for current_date in listing_availability['date']:

        # only conduct an availability calculation if the listing is
        # availaible that night. The default value of False applies
        # otherwise
                    ...
                    if listing_availability[listing_availability['date'] ==
                        ...
                        if listing_availability[listing_availability['date']
                            ...
                            if available_days == min_stay:
                                idx = availability_data.index[
                                    (availability_data['listing_id'] ==
                                     listing) & (availability_data['date'] ==
                                                 current_date)].tolist()
                                availability_data.loc[idx,
                                                      'true_availability']\
                                    = True
                                break
                        else:
...
availability_data.to_csv('calendar_true_availability.csv', index=False)
